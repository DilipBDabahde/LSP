/*
	in this application we create server application to demonstration concept of message queue
...
int main(int argc, char *argv[])
{
	...
	//msg prepartion
	msgid = msgget(key, IPC_CREAT | 0644);   //create msg q if not exists, if its there then open it
	if(msgid == -1)
	{
	...
	printf("Enter msg string: ");
	scanf("%[^'\n']s",buff);
		
	msgInfo.msgtype = 2020;
	strcpy(msgInfo.msgbuff, buff);
	
	
	//sending mgs
	ret = msgsnd(msgid,				//msg_q id which is unique 
